# Machine Learning Tutorials

This repository will house the machine learning tutorials that I find interesting and want to remember. 

## Neural Networks
1. [Introduction to Neural Networks - WildML](http://www.wildml.com/2015/09/implementing-a-neural-network-from-scratch/)
2. [Unsupervised Feature Learning and Deep Learning](http://ufldl.stanford.edu/tutorial/)
    * [Linear Regression](ufldl/ex1/linear_regression.py)
    * [Logistic Regression](ufldl/ex1/log_regression.py)
    
## Reinforecement Learning
1. [Reinforcement Learning - Part 1](http://outlace.com/Reinforcement-Learning-Part-1/)
    * [Reinforcement Learning](reinforcement_learning/reinforcement_learning-part_1.py)
